...
class VariablePressureShockTube(sim.Simulation):
    ...
    def __init__(self,pressure:float,temperature:float,observables:list,
                 ...
                 atol_sens=1e-6,volumeTrace='',exactDerivFlag=False,pressure_trace_csv=''):
        ...
        self.pressure_trace_csv = pressure_trace_csv
        ...
        self.pressure_profile_class = PressureProfile(self.pressure_trace_csv)
    ...
    def run(self,initialTime:float=-1.0, finalTime:float=-1.0):
        ...
        while t < self.finalTime:
            ...
            if mechanicalBoundary =='constant volume':
                ...
                #########################################################
                gamma = RCM.thermo.cp/RCM.thermo.cv
                current_temp = RCM.T
                current_press=RCM.thermo.P
                current_time = t
                percent_rise = .02
                pressure_measured = current_press + percent_rise*(current_time-previous_time)*current_press
                calculate_update_temp = current_temp*((pressure_measured/current_press)**((gamma-1)/gamma))
                
                
                empty_dict = {}
                for i,species in enumerate(self.processor.solution.species_names):
                    empty_dict[species] = RCM.thermo.X[i]
                    
                self.processor.solution.TPX = calculate_update_temp,pressure_measured,empty_dict      
                RCM.insert(self.processor.solution)
                previous_time = t
                ...
                #RCM.thermo.TP = calculate_update_temp,pressure_measured  

                
                #print(current_temp,RCM.thermo.T)
                #print(pressure_measured,RCM.thermo.P,RCM.thermo.TP)
                #print(calculate_update_temp,RCM.thermo.T,RCM.thermo.TP)
                ...
                # current_press=RCM.thermo.P
                # current_time = t
                # pressure_measured = self.pressure_profile_class(current_time)
                # calculate_update_temp = current_temp*((pressure_measured/current_press)**((gamma-1)/gamma))
                # RCM.thermo.TP = calculate_update_temp,pressure_measured
                #########################################################
        ...
        # temp_v.to_csv('/Users/carlylagrotta/Dropbox/Columbia/MSI/data/DME-Methanol_Blends_RCM/time_history_velocity_test/velocity_history_three.csv',index=False)
...
class PressureProfile(object):
    ...
    def __init__(self, pressureProfileCsv):
        # The time and volume are stored as lists in the keywords
        # dictionary. The volume is normalized by the first volume
        # element so that a unit area can be used to calculate the
        # velocity.
        self.df = pd.read_csv(pressureProfileCsv,float_precision='round_trip')
        self.arr = self.df.to_numpy()
        
        self.time = self.arr[:, 0]

        self.pressure = self.arr[:, 1]
        
        self.pressure =self.pressure*101325
        

        #normalize the volume
        import matplotlib.pyplot as plt
        coeffs = np.polynomial.chebyshev.chebfit(self.time, self.pressure, 15, rcond=None, full=False, w=None)
        y_calculated = np.polynomial.chebyshev.chebval(self.time, coeffs, tensor=True)
        
        #plt.scatter(self.time,self.pressure)
        #plt.plot(self.time,y_calculated)
        


        self.pressure_fit = y_calculated
    ...
    def __call__(self, t):
        ...
        if t <= self.time[-1] and t >= self.time[0]:
            ...
            return self.pressure_fit[index]
        elif t>self.time[-1]:
            return self.pressure_fit[-1]
        elif t<self.time[0]:
            return self.pressure_fit[0]        
