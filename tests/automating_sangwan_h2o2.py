...
df = pd.read_csv('MSI/data/automating_sangwan_H2O2/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    NO_mf = NO[i]
    He_mf = He[i]
    species_list = [H2O_mf,O2_mf,H_mf,OH_mf,O_mf,N2O_mf,NO_mf,He_mf]
    species_string = ['H2O','O2','H','OH','O','N2O','NO','He']
    uncertainty_string = [2.3,2.3,2.3,2.3,2.3,2,2.3,2.3]
#    
#    
    test_p = pr.Processor('MSI/data/automating_sangwan_H2O2/sangwan_one_reaction.cti')
...
#    
    ...
    OH_ppm = time_History['OH']
    time = time_History['time']
    OH_df = pd.concat([time,OH_ppm],axis=1)
    OH_df.columns = ['Time','OH_ppm']
    OH_df.to_csv('MSI/data/automating_sangwan_H2O2/'+'sangwan_oh_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_sangwan_H2O2/'+'sangwan_oh_'+str(i)+'.csv'

    
    
    
    yaml_template = 'MSI/data/automating_sangwan_H2O2/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Sangwan_'+str(i)+yamlExtention
    shutil.copy2(yaml_template,NewName)
    
    #absorbance_template = 'MSI/data/automating_lightfoot_1988/automation_template_abs.yaml'
    #tempName2 = absorbance_template[0:(len(absorbance_template)-28)]
    #absExtention = absorbance_template[(len(absorbance_template)-5):]
...
#    
    ...
    #with open(NewName2) as f:
        #config3=yaml.safe_load(f)
    #with open(NewName2,'w') as f:
...
#    working_directory = 'MSI/data/automating_O_H2_sutherland'
...
#    master_index = [2,3,4,5,6,7]
##
#    master_equation_uncertainty_df = pd.read_csv('MSI/data/automating_O_H2_sutherland/six_parameter_fit_large_uncertainty.csv')
###this could be 'On'
##
