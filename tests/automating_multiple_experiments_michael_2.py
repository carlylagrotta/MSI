...
df = pd.read_csv('MSI/data/automating_O_H2_michael/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    H_ppm = (time_History['O'][0:temp])*1e6
    time = time_History['time'][0:temp]
    H_df = pd.concat([time,H_ppm],axis=1)
    H_df.columns = ['Time','H_ppm']
    H_df.to_csv('MSI/data/automating_O_H2_michael/'+'michael_o_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_O_H2_michael/'+'michael_o_'+str(i)+'.csv'
    yaml_template = 'MSI/data/automating_O_H2_michael/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Michael_set_two_'+str(i)+yamlExtention
    ...
    files_to_include = [['Michael_set_two_'+str(i)+'.yaml']]                                                                            

    numer_of_iterations = 3
    cti_file = 'FFCM1_custom.cti'
    working_directory = 'MSI/data/automating_O_H2_michael'
...
##master_index = [2,3,4,5,6,7]
    master_index = [2,3,4,5,6,7]
#
    master_equation_uncertainty_df = pd.read_csv('MSI/data/automating_O_H2_michael/six_parameter_fit_large_uncertainty.csv')
##this could be 'On'
#
