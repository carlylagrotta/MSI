...
df = pd.read_csv('MSI/data/automating_OH_O_hong/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    slope = deltas['H2O'] / deltas['time']
    less_than_one=[]
    negative=[]
    
    window = 101
    der2 = savgol_filter(time_History['H2O']*1e6,
...
#            end_flag=ss
    ...
    plt.plot(time_History['time'][0:front_flag],time_History['H2O'][0:front_flag])
    plt.plot(time_History['time'][end_flag:time_History.shape[0]],time_History['H2O'][end_flag:time_History.shape[0]])
#            
#            
...
#    
    ...
    OH_df = pd.concat([time,H2O_ppm,Relative_uncertainty,W],axis=1)
    OH_df.columns = ['Time','H2O_ppm','Relative_Uncertainty','W']
    
    #write a column that gives time dependence to the array 
    
    
    
    OH_df.to_csv('MSI/data/automating_OH_O_hong/'+'hong_h2o_automation_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_OH_O_hong/'+'hong_h2o_automation_'+str(i)+'.csv'
    #make new automation template and add to the folder 
    yaml_template = 'MSI/data/automating_OH_O_hong/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Hong_automation_'+str(i)+yamlExtention
...
#     files_to_include = [['Hong_automation_'+str(i)+'.yaml']]                                                                            

#     numer_of_iterations = 5
#     cti_file = 'glarborg_custom.cti'
#     working_directory = 'MSI/data/automating_OH_O_hong'
...
#     #set gas original 
#     gas_original = ct.Solution('MSI/data/automating_OH_O_hong/glarborg_custom.cti')
...
#     gas_optimized = ct.Solution('MSI/data/automating_OH_O_hong/glarborg_custom_updated.cti')
