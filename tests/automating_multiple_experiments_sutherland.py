...
df = pd.read_csv('MSI/data/automating_O_H2_sutherland/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    test_p = pr.Processor('MSI/data/automating_O_H2_sutherland/one_reaction.cti')
    ...
    O_ppm = (time_History['O'][0:temp])*1e6
    time = time_History['time'][0:temp]
    O_df = pd.concat([time,O_ppm],axis=1)
    O_df.columns = ['Time','O_ppm']
    O_df.to_csv('MSI/data/automating_O_H2_sutherland/'+'sutherland_o_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_O_H2_sutherland/'+'sutherland_o_'+str(i)+'.csv'
    yaml_template = 'MSI/data/automating_O_H2_sutherland/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Sutherland_'+str(i)+yamlExtention
