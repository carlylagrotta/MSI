...
df = pd.read_csv('MSI/data/automating_HO2/experiment_conditions_just_water.csv')
...
for i in range(df.shape[0]):
    ...
    H_df.to_csv('MSI/data/automating_HO2/'+'pirraglia_h_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_HO2/'+'pirraglia_h_'+str(i)+'.csv'
    yaml_template = 'MSI/data/automating_HO2/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Pirraglia_'+str(i)+yamlExtention
    ...
    files_to_include = [['Pirraglia_'+str(i)+'.yaml']]                                                                            
