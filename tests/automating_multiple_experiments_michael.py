...
df = pd.read_csv('MSI/data/automating_H_H2O_michael/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    H2O_mf = H2O[i]
    Ar_mf = Ar[i]
    species_list = [H_mf,H2O_mf,Ar_mf]
    species_string = ['H','H2O','Ar']
    uncertainty_string = [2.3,.05,0]
    
    
    test_p = pr.Processor('MSI/data/automating_H_H2O_michael/one_reaction.cti')
    ...
    H_ppm = (time_History['H'][0:temp])*1e6
    time = time_History['time'][0:temp]
    H_df = pd.concat([time,H_ppm],axis=1)
    H_df.columns = ['Time','H_ppm']
    H_df.to_csv('MSI/data/automating_H_H2O_michael/'+'michael_h_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_H_H2O_michael/'+'michael_h_'+str(i)+'.csv'
    yaml_template = 'MSI/data/automating_H_H2O_michael/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Michael_'+str(i)+yamlExtention
