...
#df = pd.read_csv('MSI/data/automating_HO2_masten/experiment_conditions_test.csv')
df = pd.read_csv('MSI/data/automating_HO2_masten/experiment_conditions.csv')
...
for i in range(df.shape[0]):
    ...
    test_p = pr.Processor('MSI/data/automating_HO2_masten/masten_paper_gri_thermo.cti')
    ...
    deltas = time_History.diff().drop(0)
    slope = deltas['OH'] / deltas['time']
    less_than_one=[]
    negative=[]
    
    window = 101
    der2 = savgol_filter(time_History['OH']*1e6,
                         window_length=101,polyorder=2,deriv=2)
    plt.figure()
    plt.plot(der2)
    #print(der2.max())
    sorted_der = np.sort(der2)
    
    #plt.figure()
    #plt.plot(np.fft.fft(time_History['OH']))
    front_flag= None
    ...
    for ss, value in enumerate(slope):
#        if front_flag==None and der2[ss]>=der2.max(): 
#        #sorted_der[-45]:
#            front_flag=ss
#        if front_flag !=None and value<0 and end_flag ==None:
#            end_flag=ss
            
        if value >.2 and value>0 and front_flag==None:
            front_flag = ss
            less_than_one.append((ss,value))
        if value <.1 and front_flag != None and end_flag==None:
            end_flag=ss
            negative.append((ss,value))
            
    
    print(front_flag,end_flag)        
    plt.figure()
    plt.plot(time_History['time'],time_History['OH'])
    plt.plot(time_History['time'][0:front_flag],time_History['OH'][0:front_flag])
    plt.plot(time_History['time'][end_flag:time_History.shape[0]],time_History['OH'][end_flag:time_History.shape[0]])
#            
#            
#    #fall_off = find_peaks(time_History['OH'])
##    
##    #give this a shot and make sure the OH is actaully peaking 
##    temp=fall_off[0][0]
##  n  
    Relative_uncertainty = pd.DataFrame([.05]*time_History.shape[0])
    front_W =np.array([1e-10]*front_flag)
    front_W = front_W.reshape(front_W.shape[0],1)
    
    end_W = np.array([1e-10]*(time_History.shape[0]-end_flag))
    end_W = end_W.reshape(end_W.shape[0],1)
    
    count = time_History.shape[0]-(front_W.shape[0] + end_W.shape[0])
    filler_W = np.array([1/np.sqrt(count)]*count)
    filler_W = filler_W.reshape(filler_W.shape[0],1)
    
    W = pd.DataFrame(np.vstack((front_W,filler_W,end_W)))
#    
    OH_ppm = time_History['OH']*1e6
    time = time_History['time']
#    #plt.plot(time[0:front_flag],OH_ppm[0:front_flag])
#    #plt.plot(time_History['time'][end_flag:523],
#             #(time_History['OH'][end_flag:523])*1e6)
    OH_df = pd.concat([time,OH_ppm,Relative_uncertainty,W],axis=1)
    OH_df.columns = ['Time','OH_ppm','Relative_Uncertainty','W']
    
    #write a column that gives time dependence to the array 
    
    
    
    OH_df.to_csv('MSI/data/automating_HO2_masten/'+'masten_oh_'+str(i)+'.csv',index=False)
    csv_file_name = 'MSI/data/automating_HO2_masten/'+'pirraglia_oh_'+str(i)+'.csv'
    #make new automation template and add to the folder 
    yaml_template = 'MSI/data/automating_HO2_masten/automation_template.yaml'
    tempName = yaml_template[0:(len(yaml_template)-24)]
    yamlExtention = yaml_template[(len(yaml_template)-5):]
    NewName = tempName +'Masten_'+str(i)+yamlExtention
    ...
    config2['datapoints']['concentration'][0]['csvfile'] = csv_file_name
...
#    files_to_include = [['Masten_'+str(i)+'.yaml']]                                                                            
#
#    numer_of_iterations = 3
#    cti_file = 'glarborg_custom.cti'
#    working_directory = 'MSI/data/automating_HO2'
